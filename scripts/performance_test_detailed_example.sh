...
# ==============================================================================
# Comprehensive IPC/ITC Performance Test Script (v2.4 - Affinity for IPC)
#
# 功能：
...
#
# 修改重點 (v2.4):
# - 將編譯旗標變數更名為 AFFINITY_COMPILE_FLAGS。
# - [關鍵] 為 IPC 模型 (Process Producer/Consumer) 加入編譯旗標，支援 C 語言層級綁核。
#
# 用法:
...
# [修改] 使用 AFFINITY_COMPILE_FLAGS 替代原本的 THREAD_COMPILE_FLAGS
...
for bsize in "${BUFFER_SIZES[@]}"; do
    for pcount in "${PRODUCT_COUNTS[@]}"; do
        for mlen in "${MESSAGE_LENS[@]}"; do
            ...
            # 1. 編譯 [修改: 使用 AFFINITY_COMPILE_FLAGS]
            gcc "$THREAD_SRC" -o "$THREAD_EXE" -g -DNUM_PRODUCTS="$pcount" -DBUFFER_SIZE="$bsize" -DMAX_MESSAGE_LEN="$mlen" ${AFFINITY_COMPILE_FLAGS} -lpthread -lrt
            ...
            if (( pcount < PROFILING_MIN_PRODUCT_COUNT )); then
                ...
                # [修改: 重新編譯時也加入 AFFINITY_COMPILE_FLAGS]
                gcc "$THREAD_SRC" -o "$THREAD_EXE" -g -DNUM_PRODUCTS="$perf_pcount" -DBUFFER_SIZE="$bsize" -DMAX_MESSAGE_LEN="$mlen" ${AFFINITY_COMPILE_FLAGS} -lpthread -lrt
            ...
            # 1. 編譯 [修改: 加入 AFFINITY_COMPILE_FLAGS]
            gcc "${PROCESS_PRODUCER_SRC}" -o ${PROCESS_PRODUCER_EXE} -g -DNUM_PRODUCTS="$pcount" -DBUFFER_SIZE="$bsize" -DMAX_MESSAGE_LEN="$mlen" ${AFFINITY_COMPILE_FLAGS} -lpthread -lrt
            gcc "${PROCESS_CONSUMER_SRC}" -o ${PROCESS_CONSUMER_EXE} -g -DNUM_PRODUCTS="$pcount" -DBUFFER_SIZE="$bsize" -DMAX_MESSAGE_LEN="$mlen" ${AFFINITY_COMPILE_FLAGS} -lpthread -lrt
            ...
            if (( pcount < PROFILING_MIN_PRODUCT_COUNT )); then
                 perf_pcount=$PROFILING_MIN_PRODUCT_COUNT
                 # [修改: 重新編譯時也加入 AFFINITY_COMPILE_FLAGS]
                 gcc "${PROCESS_PRODUCER_SRC}" -o ${PROCESS_PRODUCER_EXE} -g -DNUM_PRODUCTS="$perf_pcount" -DBUFFER_SIZE="$bsize" -DMAX_MESSAGE_LEN="$mlen" ${AFFINITY_COMPILE_FLAGS} -lpthread -lrt
                 gcc "${PROCESS_CONSUMER_SRC}" -o ${PROCESS_CONSUMER_EXE} -g -DNUM_PRODUCTS="$perf_pcount" -DBUFFER_SIZE="$bsize" -DMAX_MESSAGE_LEN="$mlen" ${AFFINITY_COMPILE_FLAGS} -lpthread -lrt
